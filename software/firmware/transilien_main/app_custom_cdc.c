...
/** P R I V A T E  C O N S T A N T  S T R I N G S *****************************/
  ...
  #pragma  romdata
#endif
  const char strDevice[]    = "Panneau Transilien USB";
  const char strVersion[]   = "Version 0.8";
  const char strDate[]      = "22. Aout. 2015";
...
void APP_CustomCDCTasks()
{
    ...
    int8_t parameter = 0;
    int8_t command = 0;
    uint8_t need_transmit_transilien = transilien_get_tx_data(&command, &parameter);
    if (need_transmit_transilien) {
        usbInBuffer[0] = 2;
        usbInBuffer[1] = command;
        usbInBuffer[2] = parameter;
        Flags.IN_TODO = 1;        
    }
...
//------------------------------------------------------------------------------
        ...
        case CID_VERSION:// send version info-string
            ...
            break;
//------------------------------------------------------------------------------
        case CID_UPDATE_TRANSILIEN:
            if (usbOutCnt >= TRANSILIEN_DATA_LENGTH + 2)
                transilien_new_data(&usbOutBuffer[2]);            
            else {
                usbInBuffer[2] = usbOutCnt;
